...
class FlashcardUserModel(user.AbstractUserModel):
  ...
  def simulate_response(self, slate_documents):
    ...
    for i in range(0, len(slate_documents)):
      self._generate_response(slate_documents[i],
                              responses[i],
                              i+1)
  ...
  def _generate_response(self, doc, response, card_number):
    # W = np.array([1,1,1])
    doc_id = doc._doc_id
    ...
    print(f"session number: {self._user_state._time}, reviewing card number {card_number}, flashcard id {doc_id}, flashcard's recall rate = {pr}")
